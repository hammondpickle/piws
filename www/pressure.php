...
try {
  //
  $qry="SELECT (UNIX_TIMESTAMP(tstamp)-UNIX_TIMESTAMP(UTC_TIMESTAMP()))/3600.0 AS trel, pressure FROM sensor_log HAVING trel>-$tlim"; 
  $res=$dbh->query($qry);
  ...
  $graph->SetScale("linlin",900,1100,-$tlim,0);
  $graph->xaxis->SetPos('min');
  $graph->yaxis->SetPos('min');

  $aplot=new ScatterPlot($pressure, $trel);
