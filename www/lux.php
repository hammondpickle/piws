...
try {
  //
  $qry="SELECT (UNIX_TIMESTAMP(tstamp)-UNIX_TIMESTAMP(UTC_TIMESTAMP()))/3600.0 AS trel, ambient_light FROM sensor_log HAVING trel>-$tlim"; 
  $res=$dbh->query($qry);
  while(($row=$res->fetch(PDO::FETCH_ASSOC))!==false) {
    $trel[]=$row["trel"];
    $lux[]=$row["ambient_light"];
  }
  ...
  $graph->SetScale("linlog",0,0,-$tlim,0);
  $graph->xaxis->SetPos('min');
  $graph->yaxis->SetPos('min');

  $aplot=new ScatterPlot($lux, $trel);
